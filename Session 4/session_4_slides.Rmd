...
- We introduce classification by the example of the Naive Bayes which requires a better understanding the model's internal mechanisms
...
- We lay the foundation for interesting data science applications of Bayesian thinking which is nowadays widely applied
...
```{r, warning=F, R.options=list(digits=2)}
...
knitr::kable(freq_given_cat_df, align="c")
```

#### We can calculate the probability of a document blonging to a category manually  
...
<img src="../figures/P_spam_given_d.png" class="centerimg" width="82%">
...
=& `r round(P_spam_given_d,2)`
\end{align}
$$

#### Numerator

Look at the branches where the evidence (&#36;) occurs *and* the hypothesis (spam) is true; compute their joint probability P(s, &#36;)
...
Assuming independence of features allows us to easily calculate the probability of spam given **multiple** features.
...
\\=&\frac{0.75 \times0.375^2}{0.75\times 0.375^2 + 0.25\times 0.2^2} \\=& `r round(P_spam_given_d_d,2)`
\end{align}
$$
...
Recall that we have the prior of P(s)=0.75 and estimated the following conditional probabilities:
...
$$
\begin{align}
&P(\text{s}|\$, \$, \text{Paypal}, 100)\\
=&\frac{P(\text{s})P(\$|\text{s})^2 P(\text{Paypal}|s)P(100|s)}{P(\text{s})P(\$|\text{s})^2 P(\text{Paypal}|s)P(100|s) +P(\neg \text{ s})P(\$|\neg \text{ s})^2 P(\text{Paypal}| \neg s)P(100| \neg s)}
\\=&\frac{0.75 \times0.375^2 \times 0.0625^2}{0.75\times 0.375^2 \times 0.0625^2+ 0.25\times 0.2^4} \\ =& `r round(P_spam_given_d_d_100_Paypal,2)`
\end{align}
$$
...
Note how the conditional probabilities are only conditional on the message being spam but not on the features occurring in that message (independence assumption)
