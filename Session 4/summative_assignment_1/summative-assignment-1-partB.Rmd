...
Calculate the probability of a word given the category using the relative frequency of spam as the prior probability. There are a couple of ways to this. For example, you can calculate the probabilities employing `dfm()` and a bit of arithmetic. You could also estimate the Naive Bayes using `textmodel_nb()` and extract the relevant object. 
