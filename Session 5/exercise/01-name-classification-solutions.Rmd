...
d <- read.csv("../../data/EN-names.csv", stringsAsFactors=F)
...
Not terribly great. But what if we try with character n-grams up to trigrams instead of unigrams? Re-run the above analysis using trigrams. Does the classifier's performance improve?

```{r}
# tokenize the corpus but now use trigrams
characters_grams <- tokens_ngrams(characters, n = 1:3)
# create dfm
namesdfm_ng <- dfm(characters_grams)
# Naive Bayes model
nb_ng <- textmodel_nb(namesdfm_ng[train,], docvars(cnames, "gender")[train])
# predict class on test data
preds_ng <- predict(nb_ng, newdata = namesdfm_ng[test,], type = "class")
# create confusion matrix 
(cm_ng <- table(preds_ng, docvars(cnames, "gender")[test]))
# precision, recall
precrecall(cm_ng) 
# F1
2 * prod( precrecall(cm_ng)  ) / sum( precrecall(cm_ng) )
# accuracy
sum(diag(cm_ng)) / sum(cm_ng) 
```
...
library(glmnet)
set.seed(123)
lasso <- cv.glmnet(namesdfm_ng[train,], docvars(cnames, "gender")[train], 
...
```{r}
lassoPreds <- predict(lasso, namesdfm_ng[test,], type="class")
(lassoTable <- table(lassoPreds, docvars(cnames, "gender")[test]))

# precision, recall
pr <- precrecall(lassoTable[2:1, 2:1])
# F1
2 * prod(pr) / sum(pr)
# accuracy
sum(diag(lassoTable)) / sum(lassoTable)
```
...
> No, Lasso regression does not produce better predictiona than the Naive Bayes classifier with trigrams. It should be noted, however, that we only employed five-fold cross validation instead of the more commonly used ten folds. Using 10 folds might further improve the results.
