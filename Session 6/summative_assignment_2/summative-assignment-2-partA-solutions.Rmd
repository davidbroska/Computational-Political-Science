---
title: 'CPS Summative Assignment 2 Part A Solutions'
...
amicusNB <- textmodel_nb(amicusDfm, trainclass)
amicusNBpred <- predict(amicusNB)
head(amicusNBpred)
```
### Question 3 (3 points)
...
```{r}
amicusPredTable <- table(amicusNBpred, testclass)
sum(diag(amicusPredTable)) / sum(amicusPredTable)
precrecall(amicusPredTable)
precrecall(amicusPredTable[2:1, 2:1])
```
...
amicusDfmTfidf <- dfm_tfidf(amicusDfm)
# running model again and computing predictions
amicusNBTfidf <- textmodel_nb(amicusDfmTfidf, trainclass)
amicusNBpredTfidf <- predict(amicusNBTfidf)
amicusPredTableTfidf <- table(amicusNBpredTfidf, testclass)
# performance metrics
sum(diag(amicusPredTableTfidf)) / sum(amicusPredTableTfidf)
precrecall(amicusPredTableTfidf)
precrecall(amicusPredTableTfidf[2:1, 2:1])
```

**Your answer here** 

> It diminishes our precision for predicting petitioner, but improves recall; improves precision on predicting Respondent but diminishes recall.
...
Let's apply the Wordscore method to the amicus curie briefs. We start by converting the document labels into numbers. In the training vector, replace the Pro-Petitioner ("conservative") position with `-1` and the Pro-Respondent ("liberal") stance with `1`.


```{r}
docscales <- ifelse(trainclass == "P", -1, 1)
```
...
```{r}
ws <- textmodel_wordscores(amicusDfm, docscales)
scored_words <- coef(ws)

head( sort(scored_words, decreasing = T), n=10)
tail( sort(scored_words, decreasing = T), n=10)
```
...
```{r}
scored_words[ grep("race-based", names(scored_words))]
scored_words[ grep("race-conscious*", names(scored_words))]

# print first 10 rows
kwic(data_corpus_amicus, "race-based")[1:10,]
kwic(data_corpus_amicus, "race-conscious*")[1:10,]
```
