...
.pull-left[
When can look at $\frac{n-1}{2}$ pairwise comparisons between different documents
...
.pull-right[
Usually, we would continue until all items have been merged into a single cluster by the same procedure as above. 

However, there were only two clusters left which we can merge into a single one without calculations. 
