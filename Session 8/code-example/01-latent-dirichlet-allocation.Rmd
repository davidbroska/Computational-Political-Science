...
In the case of date with timestamps, looking at the evolution of certain topics over time can also help interpret their meaning. Let's look for example at Topic 23, which appears to be related to the stock market.

```{r}
# Topic 2
paste(terms[,23], collapse=", ")
sample(nyt$lead_paragraph[topics==23], 1)
...
tab <- table(nyt$year[nyt$pred_topic==23])
plot(tab)
```
...
This information is included in the matrix `gamma` in the LDA object (`theta` in the notation we used for the slides). For example, article 1 is 15% about topic 1, 5% about topics 5, 6, 8, and 21, and then less than 5% for each of the rest.

```{r}
# 5000 docs, 30 topics
dim(lda@gamma)
# probability of topic given the document
round(lda@gamma[1,], 2)
# ... sum up to 1 as they should
sum(lda@gamma[1,])
```

So we can actually take the information in the matrix and aggregate it to compute the average probability that an article each year is about a particular topic. Let's now choose Topic 29, which appears to be related to the financial crisis.

```{r}
# Topic 15
paste(terms[,29], collapse=", ")
# add probability to df
nyt$prob_topic <- lda@gamma[,29]
